...
def formula_recognition_example(image):
    ...
    formula_recognition_configs['image_resizer_path'] = './models//LaTeX-OCR_image_resizer.onnx'
    formula_recognition_configs['encoder_path'] = './models/LaTeX-OCR_encoder.onnx'
    formula_recognition_configs['decoder_path'] = './models/LaTeX-OCR_decoder.onnx'
    formula_recognition_configs['tokenizer_json'] = './models/LaTeX-OCR_tokenizer.json'
...
def document_structurization_example(image):
    ...
    layout_analysis_configs['model_path'] = './models/DocXLayout_231012.pth'  # note that: currently the layout analysis model is NOT from modelscope
...
def document_structurization_from_pdf(image_list, output_dir, base_name="page"):
    # Make sure output_dir exists
    os.makedirs(output_dir, exist_ok=True)

    # Prepare configs once
    configs = {
        'layout_analysis_configs': {
            'from_modelscope_flag': False,
            'model_path': './models/DocXLayout_231012.pth'
        },
        'text_detection_configs': {
            'from_modelscope_flag': True,
            'model_path': 'damo/cv_resnet18_ocr-detection-line-level_damo'
        },
        'text_recognition_configs': {
            'from_modelscope_flag': True,
            'model_path': 'damo/cv_convnextTiny_ocr-recognition-document_damo'
        },
        'formula_recognition_configs': {
            'from_modelscope_flag': False,
            'image_resizer_path': './models/LaTeX-OCR_image_resizer.onnx',
            'encoder_path': './models/LaTeX-OCR_encoder.onnx',
            'decoder_path': './models/LaTeX-OCR_decoder.onnx',
            'tokenizer_json': './models/LaTeX-OCR_tokenizer.json'
        }
    }
    ...
    for idx, image in enumerate(image_list):
        ...
        final_result.append({'page': idx, 'information': result})

        # Save visualization
        vis_img = document_structurization_visualization(result, image)
        cv2.imwrite(os.path.join(output_dir, f"{base_name}{idx}.png"), vis_img)
    ...
    if image is not None or image_list is not None:
        if args.task == 'general_text_reading':
            ...
            if not image_list:
                ...
                document_structurization_from_pdf(image_list, args.output_path)
